...
type SidebarItem = { id: EditorSection; label: string; icon: React.ElementType; devOnly?: boolean }
...
const mainItems: SidebarItem[] = [
  ...
  { id: "templates", label: "Templates", icon: LayoutTemplate, devOnly: true }
]
        ...
        .map((sectionItem) => {
          ...
          return (
            <Button
              ...
              } ${sectionItem.devOnly ? "opacity-40" : ""}`}
              onClick={() => onSectionChange(sectionItem.id)}
              title={sectionItem.devOnly ? `${sectionItem.label} (DEV)` : sectionItem.label}
            >
