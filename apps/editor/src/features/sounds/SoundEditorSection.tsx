    ...
    <div className="mvp15-sound-editor-container flex h-full w-full overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm">
      ...
      <aside className="flex w-[220px] flex-col border-r border-slate-200 bg-slate-50">
        <div className="flex items-center justify-between border-b border-slate-200 p-3">
          <span className="text-xs font-semibold text-slate-500 uppercase tracking-wider">{t("soundsTitle")}</span>
        </div>

        <div className="flex-1 overflow-y-auto p-2">
          <div className="flex flex-col gap-1">
            {sounds.length === 0 && (
              <p className="px-2 py-4 text-center text-xs text-slate-400">{t("soundsNoSounds")}</p>
            )}
            {sounds.map((sound) => (
              <div
                key={sound.id}
              ...
              >
                ...
                <div className="flex flex-col overflow-hidden">
                  ...
                  <span className="truncate text-[10px] text-slate-400">
                    {sound.assetSource || t("soundsNoSource")}
                  </span>
                </div>
              </div>
            ))}
          ...
          {isAdding ? (
            <div className="flex gap-2">
              <input
                ...
                placeholder={t("soundsNamePlaceholder")}
              />
              <Button
                size="sm"
                className="h-8 w-8 shrink-0 px-0"
                onClick={handleAddSound}
                title={t("soundsAddTitle")}
              >
      ...
      <div className="flex flex-1 flex-col">
        <div className="flex h-12 items-center border-b border-slate-200 px-4">
          <h3 className="text-sm font-semibold text-slate-800">{t("soundsAssetSourcesTitle")}</h3>
        </div>
                ...
                >
                  <div className="min-w-[100px]">
                    ...
                    <p className="text-[10px] text-slate-400">
                      {sound.uploadStatus === "ready" ? t("soundsStatusReady") : t("soundsStatusNotConnected")}
                    </p>
                  </div>

                  <input
                    value={sound.assetSource}
                    placeholder={t("soundsAssetPlaceholder")}
                    onChange={(e: ChangeEvent<HTMLInputElement>) =>
                  ...
                  <label className="mvp15-sound-upload-label flex h-8 cursor-pointer items-center gap-1.5 rounded-md border border-slate-200 bg-white px-3 text-xs text-slate-600 transition-colors hover:bg-slate-50 hover:text-slate-900">
                    ...
                    {uploadingSoundId === sound.id ? t("soundsUploading") : t("soundsImport")}
                    <input
                      className="hidden"
                      type="file"
                      accept=".wav,.mp3,.ogg"
                      ...
                      onChange={(event) => {
                        ...
                        void (async () => {
                          if (!validateAssetFile(selectedFile, "sound")) {
                            setValidationMessage(t("soundsInvalidFormat"))
                            inputElement.value = ""
                            return
                          }
