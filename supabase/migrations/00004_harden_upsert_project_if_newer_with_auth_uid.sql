...
declare
  ...
  v_auth_user_id uuid;
begin
  -- Keep p_user_id for backward compatibility with existing clients.
  -- Writes are always scoped to the authenticated JWT user to satisfy RLS safely.
  ...
  if v_effective_updated_at is null then
    ...
    where projects.user_id = v_auth_user_id
